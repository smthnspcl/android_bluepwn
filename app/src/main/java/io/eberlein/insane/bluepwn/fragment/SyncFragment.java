...
public class SyncFragment extends Fragment {
    ...
    BroadcastReceiver gotImportResultsReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            for(JsonElement i : results){
                c += 1;
                tableToTextView(intent.getStringExtra("table")).setText(c + " / " + results.size());
            }
        }
    };
    ...
    void importResults(String table, JsonArray results) {
        tableToTextView(table).setText("importing: " + results.size());
        JsonObject j = generateCookieRequestBody();
        j.add("uuids", results);
        Ion.with(getContext())
                .load(settings.getServer() + URL_TABLE_GET.replace(URL_TABLE_VARIABLE, table))
                .setJsonObjectBody(j)
                .asJsonArray()
    ...
    void exportResults(String table, JsonArray results) {
        tableToTextView(table).setText("exporting: " + results.size());
        JsonObject j = generateCookieRequestBody();
        ...
        Ion.with(getContext())
                .load(settings.getServer() + URL_TABLE_SET.replace(URL_TABLE_VARIABLE, table))
                .setJsonObjectBody(j)
                .asJsonObject()
    ...
    void getCookie(){
        ...
        Ion.with(getContext())
                .load(settings.getServer() + URL_AUTHENTICATE)
                .setJsonObjectBody(j)
                .asJsonObject()
                .setCallback(new FutureCallback<JsonObject>() {
                    @Override
    ...
    private JsonObject generateCookieRequestBody(){
        ...
        r.addProperty("username", settings.getUsername());
    ...
    private void difference(String table){
        ...
        Ion.with(getContext()).load(settings.getServer() + URL_TABLE_DIFFERENCE.replace(URL_TABLE_VARIABLE, table))
                .setJsonObjectBody(r)
                .asJsonObject()
                .setCallback(new FutureCallback<JsonObject>() {
                    @Override
